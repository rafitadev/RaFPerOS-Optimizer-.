...
### Notas específicas: Exynos 8890/8895 e Snapdragon 665

A detecção foi ampliada para kernels Samsung/Exynos e Qualcomm Snapdragon 665:

- **Exynos/Mali**: procura `cpu*/cpufreq`, `policy*/cpufreq`, `devfreq/*mali*`, `devfreq/*g3d*`, `/sys/devices/platform/*mali*`, `/dev/cpuset`, `/dev/stune`, `/dev/cpu.uclamp` e `/dev/cpuctl`.
- **Snapdragon 665 / Adreno 610**: procura KGSL em `/sys/class/kgsl/kgsl-3d0`, devfreq KGSL, `msm_performance` touchboost, `cpu_boost`, cpufreq policy/cpu e cpuset/stune/uclamp.
- A UI mostra **manual/testar** quando a detecção automática não encontrar endpoint, mantendo o toggle acionável; o backend decide com segurança e registra `APPLY` ou `SKIP`.
- Se uma opção ainda aparecer incompatível no aparelho, rode `rafperctl support` e envie o trecho do log; o módulo foi feito para aceitar novos paths sem hardcode por modelo.
...
A WebUI agora usa uma ponte de execução real para o backend, migra automaticamente toggles ausentes de instalações antigas e o status inclui `soc=` para ajudar no diagnóstico de Exynos/Snapdragon: primeiro tenta importar `exec` da API JavaScript do KernelSU (`kernelsu`), depois tenta fallbacks compatíveis (`window.ksu.exec` e `window.KSU.exec`). Cada toggle executa `rafperctl toggle <KEY> <0|1> apply`, ou seja: salva o estado persistente em `state/toggles.conf`, aplica imediatamente, atualiza o dashboard e mostra a saída da última ação. A WebUI não bloqueia mais toggles por detecção incompleta: quando algo não é detectado, aparece como `manual/testar`, o usuário pode tentar aplicar, e o backend registra `APPLY`, `SKIP` ou `ERROR` nos logs sem quebrar o sistema.
...
### APPLY_ON_BOOT não depende do kernel

`APPLY_ON_BOOT` é apenas um controle persistente do módulo. Ele agora é reportado como `supported` pelo backend. Além disso, a WebUI não desativa mais os demais toggles só porque a detecção automática não encontrou o endpoint; ela deixa o usuário testar e confere o resultado pelo log. Se o módulo foi atualizado por cima de uma instalação antiga, `rafperctl` mescla automaticamente novas chaves de `common/default_toggles.conf` em `state/toggles.conf`, evitando que toggles novos apareçam quebrados ou ausentes.


### Modo manual/testar na WebUI

Alguns kernels Samsung/Qualcomm escondem endpoints até um serviço inicializar, usam permissões diferentes ou expõem paths com nomes de vendor. Por isso a WebUI não usa mais “unsupported” como bloqueio. Quando a detecção não encontrar suporte, o card mostra **manual/testar** e permite ativar a opção. O backend continua seguro: ele só escreve em arquivo existente e gravável; se não der, registra `SKIP` e segue sem travar boot/SystemUI.
