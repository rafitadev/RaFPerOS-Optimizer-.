...
function labelState(s){ return s === 'unsupported' || s === 'unknown' ? 'manual/testar' : s; }
function build(values,support){ const root=document.getElementById('featureSections'); root.innerHTML=''; groups.forEach(g=>{ const sec=document.createElement('section'); sec.innerHTML=`<div class="section-head"><h2>${g.title}</h2><button class="secondary" data-reset="${g.id}">Restaurar seção</button></div><div class="cards"></div>`; const cards=sec.querySelector('.cards'); g.keys.forEach(k=>{ const sup=support[k]||{state:'unknown',note:'Sem detecção ainda'}; const checked=values[k]==='1'; const disabled=false; const card=document.createElement('div'); card.className='feature-card'+(risky.has(k)?' risky':''); card.innerHTML=`<div class="feature-head"><label><input type="checkbox" ${checked?'checked':''} ${disabled?'disabled':''}>${k}</label><span class="badge ${badgeState(sup.state)}">${labelState(sup.state)}</span></div><p>${sup.note}</p><p class="impact">${descriptions[k]||''}</p>${(sup.state==='unsupported'||sup.state==='unknown')?'<p class="risk">Não detectado automaticamente; o toggle ainda pode tentar aplicar e o log mostrará APPLY ou SKIP.</p>':''}${risky.has(k)?'<p class="risk">⚠️ Opção agressiva/manual. Ative apenas se entender o risco.</p>':''}`; card.querySelector('input').onchange=async e=>{ if(e.target.checked&&risky.has(k)&&!confirm(`${k} é agressivo. Ativar e aplicar agora?`)){ e.target.checked=false; return; } setAction(await execCmd(`${ctl} toggle ${k} ${e.target.checked?1:0} apply`)); await refresh(false); }; cards.appendChild(card); }); sec.querySelector('[data-reset]').onclick=async()=>{ setAction(await execCmd(`${ctl} reset-section ${g.id}`)); await refresh(false); }; root.appendChild(sec); }); }
